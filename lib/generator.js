...
class Generator {
  ...
  run_compile_routine_with_fallbacks(name, next) {
    // The template location.
    let template = `${this.theme_src_dir}/${name}.jade`

    // The destination for the compiled file.
    const destination = `${this.theme_build_dir}/${name}.html`

    // Check if the template file exists.
    Utils.file_exists(template, (err, exists) => {
      // If the template doesn't exist, fallback to the default one.
      if (!exists) {
        template = path.join(path.resolve(__dirname, "../templates"), name)
      }

      // Compile the template.
      Utils.compile_template(template, destination, this, err => next(err, err ? null : this))
    })
  }
